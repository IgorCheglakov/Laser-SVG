...
import type { PointElement, SVGElement, GroupElement } from '@/types-app/index'
...
export function exportToSVG(elements: SVGElement[], _width: number, _height: number): string {
  ...
  // Process elements including groups
  processElements(elements, svgLines, '    ')
 ...
 */
function processElements(elements: SVGElement[], svgLines: string[], indent: string): void {
  for (const element of elements) {
    if (element.type === 'group') {
      const groupEl = element as GroupElement
      if (!groupEl.visible) continue
      
      svgLines.push(`${indent}<g id="${element.id}" data-name="${element.name}">`)
      processElements(groupEl.children, svgLines, indent + '  ')
      svgLines.push(`${indent}</g>`)
    } else if (element.type === 'point') {
      const pointEl = element as PointElement
      if (!pointEl.points || !pointEl.visible) continue
      ...
      svgLines.push(`${indent}<path`)
      svgLines.push(`${indent}  d="${d}"`)
      svgLines.push(`${indent}  fill="none"`)
      svgLines.push(`${indent}  stroke="${stroke}"`)
      svgLines.push(`${indent}  stroke-width="${strokeWidth}"`)
      svgLines.push(`${indent}  stroke-linecap="round"`)
      svgLines.push(`${indent}  stroke-linejoin="round"`)
      
      if (element.name) {
        svgLines.push(`${indent}  id="${element.id}"`)
        svgLines.push(`${indent}  data-name="${element.name}"`)
      }
      
      svgLines.push(`${indent}/>`)
    }
  }
}
