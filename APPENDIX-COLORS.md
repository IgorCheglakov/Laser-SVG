Below are the colors that should be used in the SVG editor project as colors of shapes. The numbers and letters at the start of the line represent indices of the colors (i.e. "01" for blue color, "02" for red color etc)

00 #000000
01 #0000FF
02 #FF0000
03 #00E000
04 #D0D000
05 #FF8000
06 #00E0E0
07 #FF00FF
08 #B4B4B4
09 #0000A0
10 #A00000
11 #00A000
12 #A0A000
13 #C08000
14 #00A0FF
15 #A000A0
16 #808080
17 #7D87B9
18 #BB7784
19 #4A6FE3
20 #D33F6A
21 #8CD78C
22 #F0B98D
23 #F6C4E1
24 #FA9ED4
25 #500A78
26 #B45A00
27 #004754
28 #86FA88
29 #FFDB66
T1 #F36926
T2 #0C96D9

In the developed program the colors should be presented in the exact same order as in this document. Also in the color selection pallete each color should have corresponding index displayed.